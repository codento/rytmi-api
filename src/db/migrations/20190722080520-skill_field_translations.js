  ...
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.transaction(async (transaction) => {
      ...
      await queryInterface.sequelize.query(
        `UPDATE "skillCategory" AS s
           SET "title" = json_build_object(
             'fi', (SELECT "oldTitle" FROM "skillGroup" c WHERE c."id" = s."id"),
      ...
      await queryInterface.sequelize.query(
        ...
        `, { transaction: transaction })
      await queryInterface.sequelize.query(`ALTER TABLE "skillCategory" DROP CONSTRAINT "SkillCategories_title_key"`, { transaction: transaction })
      await queryInterface.sequelize.query(`CREATE UNIQUE INDEX skillCategory_title_fi_idx ON "skillCategory"( (title->>'fi') )`, { transaction: transaction })
      await queryInterface.sequelize.query(`CREATE UNIQUE INDEX skillCategory_title_en_idx ON "skillCategory"( (title->>'en') )`, { transaction: transaction })
