...
export const createStaticTextReplacementRequests = (cv) => {
  ...
  cv.skillLevelDescriptions.forEach((item, index) => {
    // Skip 0 level
    if (index > 0) {
      requests.push(replaceAllTextRequest(`{{ skillLevelName${index} }}`, cv.skillLevelDescriptions[index].text))
...
export const createSkillTableRequests = (skills, skillsPage) => {
  ...
  orderedSkills.forEach((skill, index) => {
    ...
    requests.push(
      ...
      modifyTableTextRequest('insertText', getSkillTableObjectId(skillPageNumber, originalSkillTableObjectId), row, 2, skill.skillLevelName)
    )
  })
